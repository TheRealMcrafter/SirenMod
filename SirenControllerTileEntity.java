		...
		if (button == "red") {
			...
			for (int i = 0; i < sirensX.size(); i++) {
				...
				if (tilee instanceof GeneralSirenTileEntity) {
					GeneralSirenTileEntity tile = (GeneralSirenTileEntity) worldObj.getTileEntity(sirensX.get(i),
							sirensY.get(i), sirensZ.get(i));
					tile.setShouldStop(true);
				} else if (tilee instanceof NuclearSirenTileEntity) {
					NuclearSirenTileEntity tile = (NuclearSirenTileEntity) worldObj.getTileEntity(sirensX.get(i),
							sirensY.get(i), sirensZ.get(i));
					tile.setShouldStop(true);
				} else if (tilee instanceof AmericanSignalT121TileEntity) {
					AmericanSignalT121TileEntity tile = (AmericanSignalT121TileEntity) worldObj
							.getTileEntity(sirensX.get(i), sirensY.get(i), sirensZ.get(i));
					tile.setShouldStop(true);
				}
			}
			...
			System.err.println(
					"Unexpected Exception in SirenControllerTileEntity(39) String 'button' is an unknown value: '"
							+ button + "'");
		}
