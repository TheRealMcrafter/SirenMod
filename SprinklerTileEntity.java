...
public class SprinklerTileEntity extends TileEntity {
	...
	public void updateEntity() {
		...
		if (!worldObj.isRemote && shouldSpawn) {
			// SirenModPacketDispatcher.sendToAllAround(new
			// SirenModPlaySoundMessage(xCoord, yCoord, zCoord, "random.fuse"),
			// this.worldObj.provider.dimensionId, xCoord, yCoord, zCoord, 120);
		...
		if (!worldObj.isRemote) {
			if (this.isRunning && this.shouldSend) {
				SirenModPacketDispatcher.sendToAllAround(
						new SirenModSpawnParticleMessage(this.xCoord, this.yCoord, this.zCoord, ""),
						this.worldObj.provider.dimensionId, this.xCoord, this.yCoord, this.zCoord, 300);
					...
					if (rand.nextBoolean()) {
						if (rand.nextBoolean()) {
							for (int i = -4; i < 4; i++) {
								for (int j = -4; j < 4; j++) {
									for (int k = -4; k < 4; k++) {
										Block block = worldObj.getBlock(this.xCoord + i, this.yCoord + j,
												this.zCoord + k);

										if (block == Blocks.fire) {
											if (rand.nextBoolean()) {
												if (rand.nextBoolean()) {
													worldObj.setBlockToAir(this.xCoord + i, this.yCoord + j,
															this.zCoord + k);
												}
											}
										}
	...
	public void spawnParticles() {
		if (worldObj.isRemote) {
			...
			Random rand = new Random();
			if (rand.nextBoolean()) {
				worldObj.spawnParticle("splash", this.xCoord + 0.6D, this.yCoord + 0.8, this.zCoord + 0.6D,
						rand.nextGaussian() / 10, 0.0D, rand.nextGaussian() / 10);
				worldObj.spawnParticle("splash", this.xCoord + 0.6D, this.yCoord + 0.8, this.zCoord + 0.6D,
						rand.nextGaussian() / 10, 0.0D, rand.nextGaussian() / 10);
				worldObj.spawnParticle("splash", this.xCoord + 0.6D, this.yCoord + 0.8, this.zCoord + 0.6D,
						rand.nextGaussian() / 10, 0.0D, rand.nextGaussian() / 10);
				...
				worldObj.spawnParticle("splash", this.xCoord + 0.6D, this.yCoord + 0.8, this.zCoord + 0.6D,
						rand.nextGaussian() / 10, 0.0D, rand.nextGaussian() / 10);
				worldObj.spawnParticle("splash", this.xCoord + 0.6D, this.yCoord + 0.8, this.zCoord + 0.6D,
						rand.nextGaussian() / 10, -0.4D, rand.nextGaussian() / 10);
			}
		}
