...
public class NuclearSirenTileEntity extends TileEntity implements ISirenTileEntity {
	...
	public void updateEntity() {
		...
		if (this.isPlaying) {
			// List l =
			// this.worldObj.getEntitiesWithinAABB(EntityVillager.class,
			// AxisAlignedBB.getBoundingBox(this.xCoord - 20, this.yCoord - 20,
			// this.zCoord - 20, this.xCoord + 20, this.yCoord + 20, this.zCoord
			// + 20));
		...
		if (!isPlaying && shouldStart) {
			...
			SirenModPacketDispatcher.sendToAllAround(
					new SirenModPlayLoopedSoundMessage(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(),
							"nuclearSiren"),
	...
	public void startSingleRotation() {
		worldObj.playSoundEffect(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(),
				"sirenmod:nuclearSiren", 8.0F, 1.0F);
		this.isOn = true;
	}
	...
	public Packet getDescriptionPacket() {
		...
		return new S35PacketUpdateTileEntity(
				new BlockPos(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ()), 1, tileTag);
	}
